...
function navigateTo(pageId) {
    ...
    if (pageId === 'conference-detail' && _conferenceCache.length === 0) {
...
function buildAdminTableHTML(moduleName, items) {
    ...
    return `
        ...
        const imgSrc = resolveMediaUrl(item.image || item.cover_image_url);
        ...
        return `
                    <tr id="admin-row-${moduleName}-${item.id}">
                        <td>${imgSrc ? `<img src="${imgSrc}" alt="" style="width:56px;height:42px;object-fit:cover;border-radius:6px;" onerror="this.style.display='none'">` : '<span style="color:#ccc;font-size:0.75rem;">—</span>'}</td>
...
function populateAdminEditForm(moduleName, item) {
    ...
    adminEditExistingImageUrl = item.image || item.cover_image_url || '';
    adminEditSelectedImageFiles = [];
    const imageEl = document.getElementById('adminEditImage');
    if (imageEl) imageEl.value = adminEditExistingImageUrl;
    renderAdminEditImageThumbnails();
...
function renderCardDetailMedia(detail) {
    ...
    if (allImages.length === 1 || image) {
        ...
        return `<div style="background:#000; width:100%; height:100%; display:flex; align-items:center; justify-content:center;"><img class="card-detail-media-el" src="${escapeHtml(singleImg)}" alt="${safeTitle}" loading="lazy" decoding="async" style="opacity:0;transition:opacity .4s" onload="this.style.opacity='1'"></div>`;
    }
...
async function loadConferencesPage() {
    try {
        ...
        renderUpcomingConferences(upcoming);
...
function renderFeaturedConference(conf) {
    ...
    wrap.innerHTML = `
        <div class="conf-featured-card" onclick='openConferenceDetail(${conf.id})'>
            ${conf.cover_image_url ? `<img src="${resolveMediaUrl(conf.cover_image_url)}" class="conf-card-image" alt="Cover">` : '<div class="conf-featured-accent"></div>'}
...
function renderUpcomingConferences(upcoming) {
    ...
    grid.innerHTML = upcoming.map(c => {
        ...
        return `
        <div class="conf-card" onclick='openConferenceDetail(${c.id})'>
            ${c.cover_image_url ? `<img src="${resolveMediaUrl(c.cover_image_url)}" class="conf-card-image" alt="Cover">` : '<div class="conf-card-top"></div>'}
...
function renderPastConferences(past) {
    ...
    timeline.innerHTML = past.map(c => {
        ...
        return `
        <div class="conf-tl-item" onclick='openConferenceDetail(${c.id})'>
            ${c.cover_image_url ? `<img src="${resolveMediaUrl(c.cover_image_url)}" class="conf-card-image" alt="Cover">` : ''}
...
function openConferenceDetail(confId) {
    const conf = _conferenceCache.find(c => c.id === confId);
    if (!conf) return;

    const wrap = document.getElementById('conf-detail-content');
    if (!wrap) return;

    const isPast = !['OPEN'].includes((conf.status||'').toUpperCase()) 
                   && (!conf.start_date || new Date(conf.start_date) <= new Date());
    const dateStr = conf.display_date || (conf.start_date 
        ? new Date(conf.start_date).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' }) 
        : 'TBA');
    
    // Auto-generate code badge from title initials + year
    const year = conf.year || (conf.start_date ? new Date(conf.start_date).getFullYear() : '');
    const codeStr = conf.title ? conf.title.split(' ').map(w => w[0]).join('').replace(/[^a-zA-Z]/g, '').slice(0, 6).toUpperCase() : 'CONF';
    const codeLabel = `${codeStr}${year}`;

    const imageUrl = conf.cover_image_url ? resolveMediaUrl(conf.cover_image_url) : (conf.image_url ? resolveMediaUrl(conf.image_url) : '');
    const statusCls = (conf.status || '').toUpperCase() === 'OPEN' ? 'open' : 'closed';
    const statusLbl = (conf.status || 'OPEN').toUpperCase();

    // The image area (placeholder if no image)
    const imageHtml = imageUrl 
        ? `<div class="conf-detail-image-wrap"><img src="${imageUrl}" alt="${conf.title}"></div>`
        : `<div class="conf-detail-image-wrap">
             <div class="conf-detail-image-placeholder">
    ...
    // Overview paragraphs
    const paragraphsHtml = conf.description 
    ...
    if (isPast) {
        if (conf.external_url) {
            ctaHtml = `<a href="${conf.external_url}" target="_blank" class="conf-detail-cta-btn ghost">
                         <i data-lucide="external-link"></i> View Proceedings
                       </a>`;
        }
    } else {
        if (conf.external_url) {
            ctaHtml = `<a href="${conf.external_url}" target="_blank" class="conf-detail-cta-btn primary">
                         <i data-lucide="external-link"></i> Register / Visit Site
                       </a>`;
        }
    }

    wrap.innerHTML = `
        <div class="conf-detail-wrap">
            <a href="#" class="conf-detail-back" onclick="navigateTo('conference'); return false;">
                <i data-lucide="arrow-left"></i> Back to conferences
            </a>
            
            <div class="conf-detail-layout">
                <!-- LEFT COLUMN -->
                <div class="conf-detail-left">
                    ${imageHtml}
                    <div class="conf-detail-code-badge">${codeLabel}</div>
                    
                    <h2 class="conf-detail-overview-heading">Conference Overview</h2>
                    <div class="conf-detail-overview-text">
                        ${paragraphsHtml}
                    </div>
                    
                    ${isPast ? `
                    <div class="conf-detail-ended-notice">
                        <i data-lucide="info"></i> This conference has ended
                    </div>` : ''}
                </div>

                <!-- RIGHT COLUMN (SIDEBAR) -->
                <div class="conf-detail-sidebar">
                    <div class="conf-detail-sidebar-label">Conference Details</div>
                    <div class="conf-detail-sidebar-title">${conf.title}</div>
                    
                    ${!isPast ? `
                    <div class="conf-detail-sidebar-status ${statusCls}">
                        <span class="conf-detail-sidebar-status-dot"></span> ${statusLbl}
                    </div>` : ''}

                    <div class="conf-detail-sidebar-divider"></div>

                    <div class="conf-detail-sidebar-row">
                        <i data-lucide="calendar"></i>
                        <div>
                            <span class="sidebar-row-label">Date</span>
                            <span class="sidebar-row-val">${dateStr}</span>
                        </div>
                    </div>
                    ...
                    <div class="conf-detail-sidebar-row">
                        ...
                        <div>
                            <span class="sidebar-row-label">Location</span>
                            <span class="sidebar-row-val">${conf.location || 'TBA'}</span>
                        </div>
                    </div>
